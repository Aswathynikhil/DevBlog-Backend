...
const generateVerificationToken = expressAsyncHandler(async (req, res) => {
  ...
  try {
    ...
    await user.save();
    //build your message
    const resetURL = `If you were requested to verify your account, verify now within 10 minutes, otherwise ignore this message <a href="https://devblogapp.netlify.app/verify-account/${verificationToken}">Click to verify your account</a>`;
    let mailOptions = {
...
const forgetPasswordToken = expressAsyncHandler(async (req, res) => {
  ...
  try {
    ...
    await user.save();
    //build your message
    const resetURL = `If you were requested to reset your password, reset now within 10 minutes, otherwise ignore this message <a href="https://devblogapp.netlify.app/reset-password/${token}">Click to Reset</a>`;
    let mailOptions = {
